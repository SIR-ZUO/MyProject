...
public class MenuSrv extends BaseSrvWithEntity<MenuEntity> {
	...
	@Transactional
	public List<TreeMenu> getMenuTree(String rootId, Set<String> allowedMenues){
		String ql = "SELECT T FROM MenuEntity T";
		...
		for (MenuEntity menu : menues) {
			TreeMenu menuTree = buildTree(menu, allowedMenues);
			if (menuTree != null) {
				ret.add(menuTree);
			}
		}
	...
	private TreeMenu buildTree(MenuEntity menu, Set<String> allowedMenues) {
		if (menu == null) {
			return null;
		}
		if ( allowedMenues.contains(menu.getResource().getId()) ) {					
			...
			if (menu.getChildren() != null && !menu.getChildren().isEmpty()) {
				...
				for (MenuEntity child : menu.getChildren()) {
					TreeMenu cTree = buildTree(child, allowedMenues);
